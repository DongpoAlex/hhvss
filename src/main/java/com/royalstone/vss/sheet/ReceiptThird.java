...
public class ReceiptThird extends SheetService {
	...
	static String	sql4Body		= " select a.sheetid,a.goodsid,a.orderqty,a.qty,a.stkqty,a.cost,a.costtaxrate, "
											...
											+ " g.manufacturer,a.BatchNo,a.ProductDate,a.InureDate,a.shopid,s.shopname "
											+ " from receiptitem a " + " join goods g on (g.goodsid=a.goodsid) "
											+ " join shop s on (s.shopid=a.shopid) " + " where a.sheetid=? ";
	static String	tableName		= "receipt";
	static String	catTableName	= "cat_receipt";
	static String	tt				= "第三方物流验收单";

	public ReceiptThird(Connection conn, Token token, String cmid) {
		super(conn, token, cmid, sql4Search, sql4Head, sql4Body, tableName, catTableName, tt);
		this.majorName = "flag";
	}
	
	public Element searchThrid(HttpServletRequest request){
		Map<String,String[]> parms = new HashMap<String, String[]>(request.getParameterMap());
		return this.search(parms );
	}
	
	public Element showThrid(HttpServletRequest request){
	...
	public Filter cookFilter(Map<String,String[]> parms) {
		...
		if(in.length()>0){
			filter.add("a.shopid IN ("+in+")");
		}else{
			throw new DAOException("第三方物流DC配置无效！");
		}
