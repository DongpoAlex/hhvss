...
public class ShowStkCostAdj {
	 ...
	 * @throws IOException 
	 */
	private Element getDetail() throws SQLException, InvalidDataException, IOException
	{
		String sql_detail = "SELECT "
			...
			+ " FROM stkcostadjitem_cdept i"
			+ " JOIN stkcostadj_cdept s ON (s.sheetid=i.sheetid)"
			+ " left join category_cdept cat ON (cat.categoryid=i.deptid)"
	...
	public int cookExcelFile(File file) throws SQLException, InvalidDataException, IOException{
		...
		String[] title = {"门店编码","门店","商品编码","品别编码","品类","条码","商品名","数量",
