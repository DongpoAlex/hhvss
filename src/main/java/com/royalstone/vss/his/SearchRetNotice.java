...
public class SearchRetNotice 
{
	...
	final static private String sql_join = " FROM  cat_retnotice_cdept c " +
			" JOIN retnotice_cdept re ON ( c.sheetid = re.sheetid ) " +
	...
	public int cookExcelFile(File file) throws SQLException, InvalidDataException, IOException{
		String[] title= {"单据号","状态","操作时间","供应商编码","供应商名称","门店编码",
			...
			int rows = excel.cookExcelFile( file, rs, title, "退货通知单明细" );
			rs.close();
			pstmt.close();
