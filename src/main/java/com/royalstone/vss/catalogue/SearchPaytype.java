...
public class SearchPaytype {
	final static private String sql_sel =  " SELECT "
											+"paytypeid,paytypename";
	final static private String sql_join = " FROM paytype ";
	public SearchPaytype ( Connection conn, Map map ) throws SQLException, InvalidDataException, IOException{
	...
	private Filter cookFilter( Map map )
	{
		...
		String[] ss = null;
		ss = (String [] ) map.get( "paytypeid" );
		if( ss != null && ss.length >0 ) {
			Values val_paytypeid = new Values ( ss );
			filter.add("paytypeid IN (" + val_paytypeid.toString4String() + ") " );
			return filter;
		}
		ss = (String [] ) map.get( "paytypename" );
		if( ss != null && ss.length >0 ) {
			Values val_paytypename = new Values ( ss );
			filter.add("paytypename IN (" + val_paytypename.toString4String() + ") " );
			return filter;
		}
		return filter;
	}
