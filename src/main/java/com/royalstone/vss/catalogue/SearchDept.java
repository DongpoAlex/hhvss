...
public class SearchDept {
	final static private String sql_sel =  " SELECT "
											+"deptid,deptname,note,touchtime,toucher";
	final static private String sql_join = " FROM dept ";
	public SearchDept ( Connection conn, Map map ) throws SQLException, InvalidDataException, IOException{
	...
	private Filter cookFilter( Map map )
	{
		...
		if( ss != null && ss.length >0 ) {
			...
			filter.add("deptid IN (" + val_shopid.toString4String() + ") " );
			return filter;
		}
		ss = (String [] ) map.get( "deptname" );
		if( ss != null && ss.length >0 ) {
			Values val_shopname = new Values ( ss );
			filter.add("deptname IN (" + val_shopname.toString4String() + ") " );
			return filter;
		}
		return filter;
	}
