 ...
 * @author liuwendong
 */
public class SearchPaymentnote{
	
	final static private String sql_sel =  " SELECT p.sheetid, status4paymentnote(p.flag) as flagname, "
		+ " p.venderid, v.vendername, (0-chargeamt) chargeamt, payamt, planpaydate, editor, p.flag, bankdate, "
		+ " nvl(ask.reqflag,0) reqflag, ask.reqdate,ask.sjprintdate,nvl(ask.sjflag,0) sjflag ";
	final static private String sql_from0 = " FROM  paymentnote0 p  " ;
	final static private String sql_from  = " FROM  paymentnote p  " ;	
	 ...
	 */
	public SearchPaymentnote ( Connection conn, Map map ) throws SQLException, InvalidDataException
	{
		this.conn = conn;
		String[] ss = (String [] ) map.get( "operation" );
		if(ss[0].equals("load_catalogue")){
			...
			String sql = sql_sel + sql_from0 + sql_join + sql_where + sql_constraint 
			+ " UNION " + sql_sel + sql_from + sql_join + sql_where + sql_constraint ;
			...
			throw new InvalidDataException( "invalid operation:" + ss[0] );
		}		
	}
	
	/**
	 * 此方法返回满足查询条件的记录总数.
	 * @param filter	a Filter obj.
	...
	private int getCount( Filter filter ) throws SQLException
	{
		...
		String sql00 = " SELECT count(*) FROM paymentnote0 p left join paymentnotevenderask ask on ask.sheetid=p.sheetid " + sql_where ;
		String sql01 = " SELECT count(*) FROM paymentnote  p left join paymentnotevenderask ask on ask.sheetid=p.sheetid " + sql_where ;
		PreparedStatement pstmt00 = conn.prepareStatement( sql00 );
		...
		if( ! rs01.next() ) throw new SQLException( "Failed in getting rows.", "", -1 );
		int rows01 = rs01.getInt( 1 );
		rs01.close();
		pstmt01.close();
		
		return ( rows00 + rows01 );
	}	

	/**
	 * 根据前台提供的参数, 设置相应的过滤条件.
	 * 目前可支持以下参数: sheetid, bookno, venderid, flag, start_date, end_date.
	 * @param map	包含查询条件的Map对象.
	 * @return	a Filter obj.
	 * @throws InvalidDataException
	 */
	private Filter cookFilter( Map map ) throws InvalidDataException
	{
		...
		if( ss != null && ss.length >0 ) {
			filter.add( " p.bookno = " + Values.toString4String(ss[0]));
		}
