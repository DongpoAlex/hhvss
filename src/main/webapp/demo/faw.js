...
/*****************************************************************
	...
	ActiveWidgets 2.0.1 Standard Edition
	Copyright (C) 2003-2006 ActiveWidgets Ltd. All Rights Reserved. 
...
AW.System.HTML.create = function () {
	...
	obj.setClass = function (name, value) {
		...
		if (element) {
			...
			var vv = v || v === 0 || v === false ? "aw-" + name + "-" + v + " " : "";
			element.className = element.className.replace(new RegExp("(aw-" + name + "-\\w* |$)"), vv);
		}
		var param = "_" + name + "Class";
		if (this[param] == null) {
			this._classes += " " + name;
...
AW.System.Control.create = function () {
	...
	obj.setAttribute("tabIndex", function () {
		...
		this[defineProperty] = function (p, v, arrayValue) {
			...
			this[get] = function (a, b, c) {
				...
				if (c !== undef && this[_p3] && this[_p3][c] && this[_p3][c][b] && this[_p3][c][b][a] !== undef) {
					...
					if (b !== undef && this[_p2] && this[_p2][b] && this[_p2][b][a] !== undef) {
						r = this[_p2][b][a];
					} else {
	...
	obj.defineModel("control", {text:"", image:"", link:"", value:controlValue, format:"", tooltip:"", state:"", visible:true});
					...
					if (e && e.getAttribute("aw")) {
						...
						if (e.getAttribute("aw") == "row" && e.id.match(/(.+-\d+-)\d+/)) {
							...
							for (x = 0; x < 3; x++) {
								z = AW.element(y + x);
								if (z) {
									z.className = z.className.replace(/ aw-mouseover-row/g, "");
								}
							}
						} else {
	...
	var handlers = {onmousemove:function (event) {
		...
		try {
			...
			while (e) {
				if (e.getAttribute && e.getAttribute("aw")) {
					raiseControlEvent("mouseDown", e, event, "aw");
					if (e.getAttribute("aw") != "control") {
		...
		try {
			...
			while (e) {
				if (e.getAttribute && e.getAttribute("aw")) {
					raiseControlEvent("mouseUp", e, event, "aw");
					if (e.getAttribute("aw") != "control") {
	...
	function focusemu(event) {
		try {
			...
			var e = event.target;
			while (e) {
				if (e.getAttribute && e.getAttribute("aw") == "control") {
					if (e != activeElement) {
						if (activeElement) {
							raiseControlEvent("onControlDeactivating", activeElement, event);
						}
						raiseControlEvent("onControlActivating", e, event);
						if (activeElement) {
							AW.object(activeElement.id).$active = false;
						}
						AW.object(e.id).$active = true;
						if (activeElement) {
							raiseControlEvent("onControlDeactivated", activeElement, event);
						}
						raiseControlEvent("onControlActivated", e, event);
						activeElement = e;
					}
...
AW.Formats.Date.create = function () {
	...
	var reverse = {"hh":["(\\d{1,2})", "a[3]=", ";"], ":mm":[":(\\d{1,2})", "a[4]=", ";"], "mm:":["(\\d{1,2}):", "a[4]=", ";"], "ss":["(\\d{1,2})", "a[5]=", ";"], "dd":["(\\d{1,2})", "a[2]=", ";"], "d":["(\\d{1,2})", "a[2]=", ";"], "mmmm":["([^\\s\\x2c-\\x2f\\x5c;]+)", "a[1]=this.rMonths[", ".toLowerCase()];"], "mmm":["([^\\s\\x2c-\\x2f\\x5c;]+)", "a[1]=this.rMonths[", ".toLowerCase()];"], "mm":["(\\d{1,2})", "a[1]=Number(", ")-1;"], "m":["(\\d{1,2})", "a[1]=Number(", ")-1;"], "yyyy":["(\\d{1,4})", "a[0]=Number(", ");if(a[0]<30){a[0]+=2000};"], "yy":["(\\d{1,4})", "a[0]=Number(", ");if(a[0]<30){a[0]+=2000};"]};
...
AW.UI.Button.create = function () {
	...
	obj.setController("button", {onControlClicked:"onClick", onKeySpace:"onClick", onKeyEnter:"onClick"});
};
...
AW.Grid.Controllers.Width = {onColumnWidthChanged:function (width, column) {
	try {
		if (this.element()) {
			this.setTimeout(function () {
				this.raiseEvent("adjustScrollWidth");
...
AW.Grid.Controllers.Virtual = (function () {
	...
	return {onSelectedRowsChanged:function () {
		calcScrollTop.call(this);
		if (AW.ie && this.element()) {
			var h = this.element().offsetHeight;
		}
	}, onCurrentRowChanged:function (i) {
		this.setTimeout(calcScrollTop);
...
AW.Grid.Controllers.MultiRow = {onKeyUp:"selectPreviousRow", onKeyDown:"selectNextRow", onKeyHome:"selectFirstRow", onKeyEnd:"selectLastRow", onKeyCtrlHome:"selectFirstRow", onKeyCtrlEnd:"selectLastRow", onKeyPageUp:"selectPageUpRow", onKeyPageDown:"selectPageDownRow", onRowClicked:"selectClickedRow", onRowCtrlClicked:"toggleClickedRow"};
...
AW.HTTP.Request.create = function () {
	...
	obj.request = function () {
		...
		if (this._requestMethod == "POST")	data = this._requestData;
...
AW.XML.Table.create = function () {
	...
	obj.transformNode = function(path){
		...
		xslDoc.async = false;
		xslDoc.load(path);
		return this._xml.documentElement.transformNode(xslDoc);
	};
	if (AW.gecko) {
		...
		obj.transformNode = function(path){
			...
			return xslProc.transformToFragment(xmlDoc, document).innerHTML;
		};
	}
};
