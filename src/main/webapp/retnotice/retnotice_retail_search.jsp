...
<xml id="island4head" />
<xml id="island4body" />
<xml id="format4head" src="<%=token.site.toXSLPatch("retnotice.xsl") %>" />
...
function show_catalogue()
{
	...
	table4search.setColumns( [ "sheetid", "status","rettype", "venderid", "vendername", "shopid", "shopname", "askshopname",
	 "retdate", "operator", "editor", "editdate", "checker", "checkdate","readtime" ] );

	var columnNames = [ "单据号", "状态", "退货类型","供应商", "供应商名称", "门店", "门店名称", "申请地",
		 "退货有效期", "业务员", "编辑人", "编辑日期", "审核人", "审核日期","供应商阅读时间" ];
	...
	for(var i=0;i<row_count;i++){
		var type = Number(grid.getCellValue(2,i));
		var labe="";
		if(type==0)
			labe="普通退货";
		else if(type==1)
			...
			labe="清品退货";
		grid.setCellText(labe,2,i);
	}
	var sumcount = table4search.getXMLNode("/xdoc/xout/rowset").getAttribute("row_total");
	div_cat.innerHTML = "<div>查询结果：总计："+sumcount+"行， 显示："+row_count+"行</div>"+grid.toString();
}
</script>
