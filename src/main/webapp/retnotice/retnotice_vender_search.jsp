...
<script language="javascript">
		...
		function cookParms(){
			...
			if(  status != '' ){
				if( Number(status) == 0 || Number(status) == 1 || Number(status) == 10 || Number(status) == 100)
...
{
	table4search.setColumns( [ "sheetid", "status","shopid", "shopname","askshopname", 
	 "retdate", "operator", "editor", "editdate", "checker", "checkdate" ] );

	var columnNames = [ "单据号", "状态","门店", "门店名称","申请地",
		 "退货有效期", "业务员", "编辑人", "编辑日期", "审核人", "审核日期" ];
...
function catchWarning(){
	...
	if( Number(warnstatus) != 100 && Number(warnstatus) != 0 ){
		var warn3  = "根据合同约定，贵司需在退货通知单发出之后的"+a1+"天内完成退货，逾期未退，需向商业有限公司交纳逾期储位占用费！";
		var warn14 = "根据合同约定，贵司需在退货通知单发出之后的"+a2+"天内完成退货,逾期未退，则视为贵司自动放弃本单所列商品及数量的所有权和处置权并承担相应法律责任！";
...
{
	var url = "../DaemonSheet?operation=doRead&clazz=Retnotice&sheetid=" + sheetid;
...
{
	var url = "../DaemonSheet?operation=doConfirm&clazz=Retnotice&sheetid=" + sheetid;
	table4confirm.setURL( url );
	table4confirm.request();
}
...
{	
	...
	var attributeOfNewWnd = "status=yes,toolbar=no,menubar=yes,resizable=1,scrollbars=1,left=0,top=0,fullscreen=0"+
 	...
 	window.open( "retnotice_print.jsp?sheetid="+current_sheetid, current_sheetid, attributeOfNewWnd );		
}
</script>
</head>
<body>
