...
<script language="javascript">
			...
			{
				var parms = new Array();
				parms.push( "sheetname=elecinstall" );
			...
			function show_catalogue()
			{
				...
				table.setColumns( [ "sheetid","status", "flag","venderid", 
				 "shopid","shopname", "saletime","operator", "editor","editdate","checker","checkdate","note" ] );
			
				var columnNames = [ "单号","状态","单据状态", "供应商编码",
				...
				obj.onCellClicked = function(event, column, row){
					if(column==0){
						open_sheet_detail( obj.getCellValue(0,row));
					}
				};
				...
				for(var i=0;i<row_count;i++){
					...
					obj.setCellText(msg,2,i);
					
					f = obj.getCellValue(1,i);
					if(f==0){
						msg="未阅读";
					}else if(f==1){
						msg="已阅读";
...
<script language="javascript">
			function load_sheet_detail( sheetid )
			{
				...
				var url = "../DaemonViewSheet?sheet=elecinstall&sheetid=" + sheetid;
				var courier = new AjaxCourier( url );
			...
			function analyse_detail( text )
			{
				...
				var isread = $("island4purchase").XMLDocument.selectSingleNode( "/xdoc/xout/sheet/head/rows/isread" ).text;
				var sheettype = $("island4purchase").XMLDocument.selectSingleNode( "/xdoc/xout/sheet/head/rows/sheettype" ).text;
				var sheetid = $("island4purchase").XMLDocument.selectSingleNode( "/xdoc/xout/sheet/head/rows/sheetid" ).text;
				var refsheetid = $("island4purchase").XMLDocument.selectSingleNode( "/xdoc/xout/sheet/head/rows/refsheetid" ).text;
				if(isread==0){
					if(sheettype==0){
						read_already(sheetid,"elecinstall");
					}else{
						read_already(refsheetid,"elecinstall");
						read_already(sheetid,"elecinstallcancel");
					}
				}
			}
			...
			function read_already( sheetid,sheetname )
			{
				var parms = new Array();
				parms.push( "sheetname="+sheetname );
				parms.push( "operation=read" );
				parms.push( "sheetid=" + sheetid );
				var url = "../DaemonSheetManager?"+parms.join("&");
				var courier = new AjaxCourier( url );
...
<script language="javascript">
			...
			{		
				var attributeOfNewWnd = "status=yes,toolbar=no,menubar=yes,resizable=1,scrollbars=1,left=0,top=0,fullscreen=0"+
			 	...
			 	window.open( "elecinstall_vender_print.jsp?sheetid="+sheetid, sheetid, attributeOfNewWnd );		
			}
		</script>
</head>
<xml id="island4result" />
<xml id="island4purchase" />
<xml id="format4sheet" src="format4elecinstall.xsl" />
<body onload="init()">
	<div id="div_tabs"></div>
	<div id="div_cat">
