...
function search_sheet( parms ){
	parms.push("clazz=PurchasechkBak");
...
function load_purchase(){
	...
	var url = "../DaemonSheet?operation=show&clazz=PurchasechkBak&sheetid=" + current_sheetid;
...
{
	//直送订单直接打开订货通知单打印界面
	if(g_logistics==1 && (g_sid==1||g_sid==2 || g_sid==5 || g_sid==6)){
		...
		var attributeOfNewWnd = "status=yes,toolbar=no,menubar=yes,resizable=1,scrollbars=1,left=0,top=0,fullscreen=0"+
	 	...
	 	window.open( "bakorder_print.jsp?sheetid="+current_sheetid, current_sheetid,attributeOfNewWnd);		
	}
}
