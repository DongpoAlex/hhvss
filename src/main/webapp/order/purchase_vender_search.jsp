...
<script language="javascript">
			...
			function analyse_catalogue( text )
			{
				...
				{    	 
					$("div_sheet").innerHTML = "";
					var elm_row = $("island4result").XMLDocument.selectSingleNode( "/xdoc/xout/rowset" );
				  	var row_count=elm_row.childNodes.length;
				  	if( row_count== "0" )
				  	{
				  		enactive_catalogue();
						$("div_sheet").innerHTML = "没有找到您要的退货通知单.";
				  	}else{
						show_catalogue();
					}
				}else{	
			...
			function show_catalogue()
			{
				...
				var node_sheetid 	= $("island4result").XMLDocument.selectNodes( "/xdoc/xout/rowset/row/sheetid" );	
				...
				var node_body 	= $("island4result").XMLDocument.selectSingleNode( "/xdoc/xout/rowset" );	
				var row_count   = node_body.childNodes.length;
				var table = new AW.XML.Table;
				table.setXML( table.getXMLContent(node_body) );
				
				table.setColumns( [ "index","sheetid", "shopid", "shopname", 
				...
				obj.setCellTemplate( obj_link, 1 );	

				obj.onCellClicked = function(event, column, row){
					var sheetid = obj.getCellValue(1,row);
				...
				$("div_sheet").innerHTML = obj.toString();
			}
		</script>
...
<script language="javascript">
			function load_sheet_detail( sheetid )
			{
				...
				var url = "../DaemonViewSheet?sheet=purchase&sheetid=" + sheetid;
				var courier = new AjaxCourier( url );
...
<script language="javascript">
			...
			{		
				var attributeOfNewWnd = "status=yes,toolbar=no,menubar=yes,resizable=1,scrollbars=1,left=0,top=0,fullscreen=0"+
			 	...
			 	window.open( "purchase_print.jsp?sheetid="+sheetid, sheetid, attributeOfNewWnd );		
			}
			//多份单据打印
			function print_all(){
...
<xml id="format4sheet"
	src="<%=token.site.toXSLPatch("format4purchase.xsl")%>" />
<body onload="init()">
	<div id="div_tabs"></div>
	<div id="div_cat">
		选择要打印单据前的复选框，点击 <input type="button" value="批量打印"
			onclick="print_all()" />
