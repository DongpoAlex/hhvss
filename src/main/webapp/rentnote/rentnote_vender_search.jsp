...
<script language="javascript">
			...
			function show_catalogue()
			{
				...
				table.setColumns( [ "sheetid", "venderid", "duedate", 
				 "shopid","shopname", "receivableamt", "receiveamt", "saleamt", "chargeamt","editor","note" ] );
			
				var columnNames = [ "单号", "供应商编码", "通知缴款日期",
					 "通知缴款地","通知缴款地", "应收款额", "已收款额", "销售金额", "费用金额","制单人","备注" ];
...
<script language="javascript">
			function load_sheet_detail( sheetid )
			{
				...
				var url = "../DaemonViewSheet?sheet=rentnote&sheetid=" + sheetid;
				var courier = new AjaxCourier( url );
...
<script language="javascript">
			...
			{		
				var attributeOfNewWnd = "status=yes,toolbar=no,menubar=yes,resizable=1,scrollbars=1,left=0,top=0,fullscreen=0"+
			 	...
			 	window.open( "rentnote_vender_print.jsp?sheetid="+sheetid, sheetid, attributeOfNewWnd );		
			}
		</script>
</head>
<xml id="island4result" />
<xml id="island4purchase" />
<xml id="format4sheet" src="format4rentnote.xsl" />
<body onload="init()">
	<div id="div_tabs"></div>
	<div id="div_cat">
