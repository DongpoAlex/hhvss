...
<script language="javascript">
		...
		var current_sheetid 	= "";
		var g_status = -1;
		var format_show_xsl = "<%=token.site.toXSLPatch("ret.xsl") %>";
		...
		function cookParms(){
			...
			parms.push("clazz=Ret");
			parms.push("operation=search");
			
			if(  $F('txt_retdate_min')  != '' )   parms.push( "retdate_min="  + $F('txt_retdate_min') );
			if(  $F('txt_retdate_max')  != '' )   parms.push( "retdate_max="  + $F('txt_retdate_max') );
			if(  $F('txt_status')  != '' )   parms.push( "status="  + $F('txt_status') );
			if(  $F('txt_venderid')  != '' )   parms.push( "venderid="  + $F('txt_venderid') );
		...
		{
			table4search.setColumns( [ "sheetid", "statusname", "rettype","shopid", "shopname","shopname", 
			 "retdate", "editor", "editor", "editdate", "retchecker", "retcheckdate" ] );
		
			var columnNames = [ "单据号", "状态", "退货类型","门店", "门店名称","申请地",
				 "供应商最迟拉货日期", "业务员", "编辑人", "编辑日期", "审核人", "审核日期" ];
		...
		{
			var url = "../DaemonSheet?operation=doRead&clazz=Ret&sheetid=" + sheetid;
		...
		{
			var url = "../DaemonSheet?operation=doConfirm&clazz=Ret&sheetid=" + sheetid;
			table4confirm.setURL( url );
			table4confirm.request();
		}
		...
		{	
			...
			var attributeOfNewWnd = "status=yes,toolbar=no,menubar=yes,resizable=1,scrollbars=1,left=0,top=0,fullscreen=0"+
		 	...
		 	window.open( "ret_print.jsp?sheetid="+current_sheetid, current_sheetid, attributeOfNewWnd );		
		}
		</script>
</head>
<body>
	...
	<div id="div0" class="search_main">
		<div class="search_parms">
			供应商编码:<span style="color: red; font: bold;">*</span> <input
				type="text" id="txt_venderid" size="10" />
		</div>
		<div class="search_parms">
			退货地:<input type="text" id="txt_shopid" size="12" /><a
		...
		<div class="search_parms">
			退货日期: <input type="text" id="txt_retdate_min"
				onblur="checkDate(this)" size="12" /> <img
				src="../img/datePicker.gif" align="absmiddle"
				style="cursor: pointer"
				onclick="WdatePicker({el:'txt_retdate_min'})" /> - <input
				type="text" id="txt_retdate_max" onblur="checkDate(this)" size="12" />
			<img src="../img/datePicker.gif" align="absmiddle"
				style="cursor: pointer"
				onclick="WdatePicker({el:'txt_retdate_max'})" />
		</div>
		<div class="search_parms">
			单据号: <input type="text" id="txt_sheetid" size="20" />
		</div>
