...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="output_head">
		...
		<table width="99%" cellspacing="0" cellpadding="2" border="1">
			...
			<tr>
				...
				<td>本期进货</td>
				<td class="num-fmt">
					<xsl:value-of select="format-number(n5,'#0.00')" />
				</td>
				<td>进货调整</td>
				<td class="num-fmt">
					<xsl:value-of select="format-number(n6,'#0.00')" />
				</td>
				<td>供应商折扣</td>
				<td class="num-fmt">
		...
		<table width="99%" cellspacing="0" cellpadding="2" border="1">
			...
			<xsl:for-each select="row">
				<tr>
					...
					<td class="num-fmt">
						<xsl:value-of select="format-number(n17,'#0.00')" />
					</td>
					<td class="num-fmt">
						<xsl:value-of select="format-number(n18,'#0.00')" />
					</td>
			...
			<tr>
				...
				<td class="num-fmt">
					<xsl:value-of select="format-number(sum(row/n17),'#0.00')" />
				</td>
				<td class="num-fmt">
					<xsl:value-of select="format-number(sum(row/n18),'#0.00')" />
				</td>
				<td class="num-fmt">
					<xsl:value-of select="format-number(sum(row/n19),'#0.00')" />
				</td>
				<td class="num-fmt">
					<xsl:value-of select="format-number(sum(row/n20),'#0.00')" />
				</td>
			</tr>
		</table>
	</xsl:template>
	...
	<xsl:template name="output_body6">
		<table width="99%" cellspacing="0" cellpadding="2" border="1">
			<caption> <h3>回执单</h3></caption>
			<xsl:for-each select="/xdoc/xout/sheet/head/row">
				<tr center="left">
					<td>供应商:[
						<xsl:value-of select="supid" />
						]
						<xsl:value-of select="c5" /></td>
					<td>结算单号:</td>
					<td><xsl:value-of select="billno" /></td>
					<td>结算方式:</td>
					<td><xsl:value-of select="n25" /></td>
					<td>结算地点：</td>
					<td><xsl:value-of select="mrid" />
						<xsl:value-of select="mridname" /></td>
				</tr>
			</xsl:for-each>
			<tr>
				<th>收取机构</th>
