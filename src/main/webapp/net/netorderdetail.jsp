...
.aw-column-0,.aw-column-1,.aw-column-7 {
	width: 80px;
	cursor: pointer;
	text-align: center
}
...
.aw-column-2,.aw-column-3,.aw-column-5,.aw-column-10,.aw-column-11 {
	width: 60px;
	text-align: center;
}

.aw-column-4,.aw-column-6,.aw-column-8 {
	width: 120px;
	text-align: left;
}
...
<script language="javascript" type="text/javascript">
 function search_sheet(){
	    ...
	    setLoading(true);
		var parms = new Array();
		parms.push("action=netorderdetail");
		...
		table.setURL(url);
		table.setTable("xdoc/xout/netorderdetail");
		table.setRows("row");
		table.request();
 ...
 function getGrid(table) {
		...
		table.setColumns([ "order_serial","request_date","start_time","end_time","po_no","supplier_no","vendername","barcode","goodsname","pkgqty","logistics","floor","rgst_date"]);
		var columnNames = ["预约流水号", "预约送货日期", "开始时间", "结束时间","订单号", "供应商编码","供应商名称","商品条码","商品名称","箱数","物流模式","楼层","VSS预约时间" ];
		grid.setId("grid");
		...
		for ( var i = 0; i < row_count; i++) {
			...
			if (grid.getCellText(10, i) == 1) {
				lable = "直送";
				grid.setCellText(lable, 10, i);
			}
			if (grid.getCellText(10, i) == 2) {
				lable = "直通";
				grid.setCellText(lable, 10, i);
			} else if (grid.getCellText(10, i) == 3) {
				lable = "配送";
				grid.setCellText(lable, 10, i);
			}
		}
		grid.refresh();
		// 数据统计信息
		var totalCount = table.getXMLText("xdoc/xout/netorderdetail/totalCount");
 ...
 function downExcel(){
	 	...
	 	var parms = new Array();
		parms.push("action=netorderdetailExcel");
