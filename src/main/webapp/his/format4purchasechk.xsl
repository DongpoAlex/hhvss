...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="output_sheet">
		...
		<xsl:if
			...
			<xsl:element name="br" />
			<xsl:for-each select="body[ @name='purchaseitem' ]">
		...
		<xsl:element name="table">
			...
			<xsl:element name="tr">
				...
				<xsl:element name="th">
					...
					整单订货数箱量汇总：
					<xsl:value-of
						select="format-number(sum(/xdoc/xout/sheet/body[ @name='purchaseitem' ]/row/pkqty),'#.')" />
					箱
				</xsl:element>
			</xsl:element>
			<xsl:element name="tr">
			...
			<xsl:for-each select="row">
				<xsl:element name="tr">
					...
					<xsl:element name="td">
						<xsl:value-of select="format-number(number(pkqty),'#.')" />
						箱
					</xsl:element>
					<xsl:element name="td">
					</xsl:element>
