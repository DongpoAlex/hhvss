...
<script language="javascript">
	...
	function cook_search_prams() {
		...
		if (txt_shopid.value != '')
			parms.push("shopid=" + txt_shopid.value);
		...
		if (txt_checkdate_min.value != '')
			parms.push("checkdate_min=" + txt_checkdate_min.value);
		if (txt_checkdate_max.value != '')
			parms.push("checkdate_max=" + txt_checkdate_max.value);
		if (txt_deadline_min.value != '')
			parms.push("deadline_min=" + txt_deadline_min.value);
		if (txt_deadline_max.value != '')
			parms.push("deadline_max=" + txt_deadline_max.value);
		if (txt_release_min.value != '')
			parms.push("releasedate_min=" + txt_release_min.value);
		if (txt_release_max.value != '')
			parms.push("releasedate_max=" + txt_release_max.value);
		if (order_status.value != '')
			parms.push("status=" + order_status.value);
		return parms;
	}
	//处理查询结果
	function analyse_search(text) {
		...
		var node_sheetid = table4search.getXML().selectNodes(
				"/xdoc/xout/catalogue/row/sheetid")
		...
		if (arr_sheetid != null && arr_sheetid.length > 0)
			current_sheetid = arr_sheetid[0];
	}
	//将查询结果转换为grid
	function result_grid() {

		table4search.setColumns([ "sheetid", "status", "purchasetype",
				"validdays", "orderdate", "venderid", "vendername",
				"majorname", "paytypename", "deadline", "logistics", "note",
				"editor", "editdate", "checker", "checkdate", "readtime" ]);
		var columnNames = [ "订货审批单号", "状态", "补货标识", "有效期（天）", "订货日期", "供应商编码",
				"供应商名称", "课类", "结算方式", "截止日期", "物流模式", "备注", "制单人", "制单日期",
				"审核人", "审核日期", "供应商阅读时间" ];
		...
		grid_link.setEvent("onclick", function() {
		...
		var sumcount = table4search.getXMLNode("/xdoc/xout/catalogue")
				.getAttribute("count");
		return "查询结果：总计：" + sumcount + "行， 显示：" + row_count + "行"
				+ grid.toString();
	}
	...
	function load_purchase() {
		if (current_sheetid == null || current_sheetid.length == 0)
			return;
		var url = "../DaemonViewHisSheet?sheet=purchasechk&sheetid="
				+ current_sheetid;
		...
		table4detail.response = function(text) {
			...
			div_sheethead.innerHTML = table4detail.getXML().transformNode(
					format4head.documentElement);
			setLoading(false);
			//记录送货模式和订货通知单号
			g_logistics = table4detail
					.getXMLText("xdoc/xout/sheet/head/row/logistics");
			var node_row = table4detail.getXML().selectNodes(
					"xdoc/xout/sheet/body[ @name='purchase' ]/row");
	...
	function sheet_navigate(step) {
		...
		if (offset >= arr_sheetid.length) {
			alert("已经是最后一份单据!");
			return false;
		}
		//显示位置
		offset_current.innerText = "当前第：" + (offset + 1) + "单，共有："
				+ arr_sheetid.length + "单";
	...
	function shop_sheet_navigate(step) {
		...
		//显示位置
		shop_offset_current.innerText = "当前第：" + (offset + 1) + "单，共有："
				+ arr_sheetid.length + "单";
	...
	function open_win_print() {
		//直送订单直接打开订货通知单打印界面
		if (g_logistics == 1) {
			var url = "purchase_vender_print.jsp?sheetid="
					+ g_orderid.join(",");
			...
			window.open("order_vender_print.jsp?sheetid=" + sheetid, sheetid,
					attributeOfNewWnd);
		}
	}
	...
	function show_confirm_catalotue() {
		...
		table4search.setColumns([ "", "sheetid", "status", "validdays",
				"orderdate", "majorname", "logistics", "note", "releasetime" ]);
		var columnNames = [ "", "订货审批单号", "状态", "有效期（天）", "订货日期", "课类", "物流模式",
				"备注", "上传时间" ];
		...
		div_confirm4all.innerHTML = "<div>查询结果：" + row_count + "条</div>$tag_"
				+ grid.toString();
	}
	...
	function shopprint_grid(text) {
		...
		table4shopprint.setColumns([ "index", "refsheetid", "sheetid",
				"shopid", "shopname", "logisticsname", "orderdate", "deadline",
				"editor", "checker" ]);

		var columnNames = [ "", "订货审批单号", "订货通知单号", "门店", "门店名称", "送货方式",
				"订货日期", "截止日期", "编辑人", "审核人" ];
	...
	function downExcel() {
		...
		parms.push("operation=order_retail");
	...
	function downloadsheet() {
		var url = "../DaemonDownloadHisExcel?sheetid=" + current_sheetid
				+ "&operation=order_sheet";
		window.location.href = url;
</script>
		...
		<div id="div_search">
			<table cellspacing="1" cellpadding="2" width="70%"
				...
				<tr>
					...
					<td><input type="text" id="txt_venderid" size="23" /></td>
					<td></td>
					<td></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class=altbg2><select id="order_status">
							...
							<option value="100">已执行</option></td>
				</tr>
