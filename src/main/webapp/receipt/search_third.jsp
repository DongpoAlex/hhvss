...
<script language="javascript">
		...
		{
			if(!check()){return;}
			var url = "../DaemonSheet?operation=searchThrid&clazz="+clazz+"&" + cookParms().join( '&' );
		...
		function load_sheet_detail( sheetid )
		{
			...
			var url = "../DaemonSheet?operation=showThrid&clazz="+clazz+"&sheetid=" + sheetid;
			...
			table4detail.response = function(text){
				...
				if( table4detail.getErrCode() != 0 ){	//处理xml中的错误消息
					$("div_sheetshow").innerHTML = table4detail.getErrNote() ;
					return;
				}
...
<body>
	...
	<div id="div0" class="search_main">
		...
		<div class="search_parms">
			验收门店:<input type="text" id="txt_shopid" size="12" name="txt_parms"
				split="," /><a href="javascript:showShopListThridDC('txt_shopid')">选择门店</a>
		</div>
		<div class="search_parms">
			验收日期: <input type="text" id="txt_editdate_min" class="Wdate"
