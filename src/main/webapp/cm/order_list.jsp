	...
	<div id="divSearch" class="search_main">
		...
		<div class="search_parms">
			供应商编码: <input type="text" id="txt_venderid" name="txt_parms"
				notnull="notnull" alt="供应商编码" split="," />多供应商逗号分隔
		</div>
		<% } %>
		<div class="search_parms">
			门店:<input type="text" id="txt_shopid" size="12" name="txt_parms"
				split="," /><a href="javascript:showShopList('txt_shopid')">选择门店</a>
		</div>
		<div class="search_parms">
			商品编码: <input type="text" id="txt_goodsid" name="txt_parms" />
		</div>
		<div class="search_parms">
			<span class="required">*</span>审核日期范围（60天内）: <input type="text"
				id="txt_checkdate_min" class="Wdate"
				onFocus="WdatePicker({minDate:'#F{$dp.$D(\'txt_checkdate_max\',{d:-60});}',maxDate:'#F{$dp.$D(\'txt_checkdate_max\',{d:0});}'})"
				name="txt_parms" notnull="notnull" alt="最小审核日期" /> - <input
				type="text" id="txt_checkdate_max" class="Wdate"
				onFocus="WdatePicker({minDate:'#F{$dp.$D(\'txt_checkdate_min\',{d:0});}',maxDate:'#F{$dp.$D(\'txt_checkdate_min\',{d:60});}'})"
				name="txt_parms" notnull="notnull" alt="最大审核日期" />
		</div>
