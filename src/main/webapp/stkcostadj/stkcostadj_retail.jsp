...
<%
	...
	sel_book.setAttribute( "id", "txt_book" );
%>
...
<script language="javascript">
		...
		*/
		function load_catalogue( parms )
		{
				...
				var url = "../DaemonStkCostAdj?" + parms.join( '&' );
		...
		{		
			var attributeOfNewWnd = "status=yes,toolbar=no,menubar=yes,resizable=1,scrollbars=1,left=0,top=0,fullscreen=0"+
		 	...
		 	window.open( "stkcostadj_print.jsp?sheetid="+sheetid, sheetid,attributeOfNewWnd);		
		}
		
		function downExcel(){
		
		var url = "../DaemonDownloadExcel?operation=stk&sheetid=" + current_sheetid;
...
<body onload="init()">
	...
	<div id="div001">
		<div id="div_detail" style="display: none;">
			...
			<div id="div_navigator">
				<input type="button" value="上一单" onclick="sheet_navigate(-1)" /> <input
					...
					onclick="javascript:downExcel()" type="button" value="导出明细" />
			</div>
			<br />
		...
		<div id="div_search" style="display: none;">
			<div class="border">
				<h5>组合条件查询</h5>
				供应商编码：<input id="txt_venderid" value="" type="text" size="15" />
				发生门店：<%=sel_branch%>
